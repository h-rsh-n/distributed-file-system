...
class NameServerService(rpyc.Service):
  ...
  # parse absolute path corresponding to the /yadfs disk in storage servers
  def absolute_path(self, path):
    # check if already begins with /yadfs
    if path[:1] != "/": path = "/" + path
    if path[:6] != self.ABSOLUTE_ROOT:  path = os.path.join(self.ABSOLUTE_ROOT, path)
    return path
  ...
  def mark_server_alive(self, ss_id):
    print(ss_id)
    if ss_id not in self.ss_blocks_map:
  ...
  def check_aliveness(self, check_count):
    ...
    for name, value in self.ss_blocks_map.items():
      ...
      if this_ss_is_alive(ss[0], ss[2]):
        print('Datanode {} is "alive".'.format(name))
        if int(value[0]) == 0:
